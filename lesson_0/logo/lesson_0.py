...
code_lines = [
    ...
    [("#F8F8F2", "{")],
    [("#F8F8F2", "    std::"), ("#50FA7B", "cout"), ("#F8F8F2", " <<")],
    [("#F1FA8C", '    "Hello, I use')],
    [("#F1FA8C", '     Linux btw!"'), ("#F8F8F2", ";")],
    [],
...
pp_w = int(draw.textlength("++", font=cpp_fnt))

c_bb  = draw.textbbox((0,0), "C",  font=cpp_fnt)
pp_bb = draw.textbbox((0,0), "++", font=cpp_fnt)

c_h  = c_bb[3] - c_bb[1]
pp_h = pp_bb[3] - pp_bb[1]
...
draw.text((RX, rule_y + 18), "ИЗУЧЕНИЕ С НУЛЯ", font=sub_fnt, fill=WHITE)
...
out = "/mnt/user-data/outputs/lesson0.png"
img.save(out, "PNG")
print("Done:", out)